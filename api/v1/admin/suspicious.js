...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── migrate_photos_list ──────────────────────────────
    // Vrátí účty, které mají ještě nezmigrované fotky (na Supabase Storage).
    if (action === 'migrate_photos_list') {
      const r = await dbQuery(`user_card_photos?select=user_id,storage_path&limit=10000`);
      const rows = await r.json();
      const perUser = {};
      (rows || []).forEach(row => {
        const sp = row.storage_path || '';
        const isSupabase = sp && !sp.startsWith('http'); // Supabase = relativní cesta
      ...
      let names = {};
      if (ids.length) {
        const pr = await dbQuery(`profiles?id=in.(${ids.join(',')})&select=id,username,email`);
    ...
    // ── migrate_account_photos ───────────────────────────
    // Přemigruje fotky JEDNOHO účtu: stáhne ze Supabase → nahraje na R2 → přepíše URL.
    // Service klíč zůstává na serveru; admin jen určí který účet a kolik (dávka).
    if (action === 'migrate_account_photos' && method === 'POST') {
      const targetUser = body?.user_id;
      const batch = Math.min(parseInt(body?.batch) || 50, 200);
      if (!targetUser) return jsonError(res, 400, 'Missing user_id');

      const R2_WORKER = 'https://pokedb-api.poketrade.workers.dev';
      const r = await dbQuery(`user_card_photos?user_id=eq.${targetUser}&select=id,storage_path,url&limit=${batch}`);
      const rows = await r.json();
      ...
      for (const row of todo) {
        try {
          ...
          if (!img.ok) { failed++; continue; } // soubor už neexistuje → přeskoč
          const arrBuf = await img.arrayBuffer();
          const ct = img.headers.get('content-type') || 'image/jpeg';
          const fname = row.storage_path.split('/').pop();
          // Nahraj na R2 — server-side použijeme admin secret workeru (ne uživatelský token)
          ...
          await dbQuery(`user_card_photos?id=eq.${row.id}`, {
            method: 'PATCH',
            body: JSON.stringify({ storage_path: upData.url, url: upData.url }),
          });
          await dbQuery(`photo_queue?storage_path=eq.${encodeURIComponent(row.storage_path)}`, {
